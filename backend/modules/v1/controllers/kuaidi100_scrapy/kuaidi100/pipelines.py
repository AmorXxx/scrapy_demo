...
class MySQLCommand(object):
    ...
    def insertData(self, my_dict):
        sqlExist = "SELECT * FROM detail  WHERE package_id ='%s 'AND time = '%s'" % (
            my_dict['package_id'], my_dict['time'])
        ...
        if res:  # res为查询到的数据条数如果大于0就代表数据已经存在
            ...
            if Rep == my_dict:  # 查询数据库内的是不是最新信息
                # 删除旧的数据
                sqlDel = "DELETE FROM  detail WHERE package_id ='%s ' AND time = '%s' AND context = '%s'" % (
                    my_dict['package_id'], my_dict['time'], my_dict['context'])
